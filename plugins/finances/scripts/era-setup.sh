...
# Find a free TCP port in 8765-8774 for the OAuth callback listener.
# Fixes silent failures when another local tool (e.g. claude-anywhere relay)
# is already bound to 8765 — the browser redirect would hit that process
# instead of our nc listener, and the auth flow would appear to just "close."
pick_callback_port() {
...
CALLBACK_PORT=$(pick_callback_port) || {
    echo "Error: No free port in range 8765-8774." >&2
    echo "Check what is holding these ports: lsof -iTCP:8765-8774 -sTCP:LISTEN" >&2
    exit 1
}

if [ "$CALLBACK_PORT" != "8765" ]; then
    HOLDER=$(lsof -iTCP:8765 -sTCP:LISTEN -Fc 2>/dev/null | grep '^c' | head -1 | cut -c2-)
    echo "Port 8765 is in use${HOLDER:+ by $HOLDER} — using port $CALLBACK_PORT instead."
fi
