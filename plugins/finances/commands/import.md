---
description: Import CSVs and manage financial context — configure what flows into /finances:ask
allowed-tools: Bash, Read, Write, Glob, Grep, AskUserQuestion
---

# Finance Import

Bring outside financial data into the workspace and manage the context file that shapes how `/finances:ask` reasons about it.

Most bank and credit card data should flow through Era (via `/finances:ask`). This command handles the stuff Era can't see: CSV exports, manual accounts, goals, and notes.
...
1. Ask where the file is (or check `finances/imports/` if they dropped one there)
2. Read it, identify columns (date, description, amount, category — these vary by bank)
3. Save the original under `finances/imports/` and a normalized version under `finances/transactions/YYYY-MM-source.csv` (e.g. `2026-04-venmo.csv`)
4. Append to `finances/context.md` under `## Data Sources` so `/finances:ask` knows it exists
...
1. Tell the user: "To add a new bank, open Era and use their Add Account flow — it handles Plaid auth and the connection will show up here next time you run `/finances:ask`."
...
Read `finances/context.md` (or `ops/finances/context.md`), then walk through:
...
Write updates back to the file. Keep it short and scannable — this gets loaded into every `/finances:ask`.
...
Then list any CSVs under `finances/transactions/`. Flag gaps: "Era sees Chase checking + Chase card. If you have savings, investment, Venmo, or PayPal not listed, we can import those via CSV or you can add them through Era."

## After any import

Append a line under `## Data Sources` in `finances/context.md`:
...
- After a successful import, offer to run `/finances:ask` to show the new data in action.
