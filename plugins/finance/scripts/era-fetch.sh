...
refresh_tokens() {
    if [ -z "$REFRESH_TOKEN" ] || [ ! -f "$CLIENT_FILE" ]; then
        reauth
        return
    fi
    CLIENT_ID=$(cat "$CLIENT_FILE" | json_get client_id)

    TOKEN_RESP=$(curl -sS --max-time 15 -X POST https://forge.era.app/oauth/token \
        -H "Content-Type: application/x-www-form-urlencoded" \
        -d "grant_type=refresh_token&refresh_token=${REFRESH_TOKEN}&client_id=${CLIENT_ID}" 2>&1) || {
        echo "Refresh request failed: $TOKEN_RESP" >&2
        reauth
        return
    }
...
mcp_call() {
    ...
    RESP=$(curl -sS --max-time 30 -w "\n%{http_code}" -X POST "$ERA_URL" \
        ...
        echo "Era request failed (network/timeout)" >&2
        exit 1
    }
...
mcp_init() {
    mcp_call "initialize" '{"protocolVersion":"2025-03-26","capabilities":{},"clientInfo":{"name":"delegate-finance","version":"0.1.0"}}' 0 >/dev/null
}
